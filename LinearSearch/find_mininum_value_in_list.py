...
def min_val_in_list(element_list):
    '''
    Function return the minimum value in the list if the list is not empty.
...
result, index = min_val_in_list(element_list)

if result:
    print(f"Minimum element in the list {element_list} is {result} found at index {index}")
else:
...
# call the linear search function to search the element
result, index = min_val_in_list(word)

if result:
    print(f"Minimum char in the string {word} is {result} found at index {index}")
else:
